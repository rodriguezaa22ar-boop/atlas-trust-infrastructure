...
2. Inspect `docs/case-studies/CASE_STUDY_RELEASE_TRUST.md` and
   `docs/case-studies/CASE_STUDY_VENDOR_PAYMENT_CHANGE.md`.
3. Inspect the synthetic reviewer demo in `docs/demo/DEMO_REVIEWER_RUNBOOK.md`.
4. Generate or inspect the reviewer package with
   `./tools/atlas/bin/atlas reviewer package atlas-current-review`.
5. Run `./tools/atlas/bin/atlas release verify <release-packet> --commit <commit>`.
6. Run `./tools/atlas/bin/atlas release manifest-verify <manifest> --commit <commit>`.
7. Run `./tools/atlas/bin/atlas release replay <release-packet> --json`.
8. Run `./tools/atlas/bin/atlas production status --strict --explain`.
9. Run `gh attestation verify <artifact>.tar.gz --repo rodriguezaa22ar-boop/atlas-trust-infrastructure`.
10. Run `slsa-verifier verify-artifact <artifact>.tar.gz --provenance-path <artifact>.intoto.jsonl --source-uri github.com/rodriguezaa22ar-boop/atlas-trust-infrastructure --source-tag <tag>` when official generic provenance exists.
11. Inspect `docs/KNOWN_LIMITATIONS.md`, the package README, and non-guarantees
