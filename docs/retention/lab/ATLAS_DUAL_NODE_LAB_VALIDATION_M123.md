...
M123 retains the dual-node lab model as an explicit,
metadata-only operational proof chain. It documents how a human operator can
use a cockpit/control node, a terminal-first builder node, a private network
path, and GitHub verification to review Atlas evidence without expanding Atlas
beyond the proof layer.
...
| --- | --- | --- |
| Cockpit/verifier/control node | Human-controlled command and review plane | Not an Atlas-operated control plane |
| Builder node | Terminal-first builder and reviewer execution node | Not an Atlas-managed runtime |
| SSH/tmux | Human-operated control session and resumable terminal surface | Not orchestration or autonomous operation |
| Private network path | Recovery/control network path for reaching the builder | Not a production security guarantee |
...
- Cockpit/control host: private lab host label redacted
- Builder/review host: private lab host label redacted
- Builder access model: SSH from cockpit to builder
- Builder terminal model: tmux session label redacted
- Recovery/control network: private network reachability
...
```bash
ssh <builder-host> -t "tmux new -A -s <tmux-session>"
```
...
```bash
ssh <builder-host> 'tmux has-session -t <tmux-session> 2>/dev/null || tmux new-session -d -s <tmux-session>; tmux display-message -p -t <tmux-session> "#{session_name}:#{session_windows}"'
```
...
Confirm private network reachability when recovery/control network validation
is in scope:

```bash
<private-network-status-command>
```
...
- A cockpit/verifier/control node role is documented.
- A terminal-first builder node role is documented.
- SSH/tmux is documented as the human-operated control plane.
- A private network path is documented as a recovery/control path.
...
- The `atlas-dual-node-cockpit` operation state is local metadata unless a
  ...
  it.
- Private network reachability proves node connectivity for the reviewed
  moment; it does not prove production availability, network security, or
...
- not a claim that Atlas controls the cockpit node, builder node, SSH, tmux,
  private networking, GitHub, or the network

## Metadata Boundary

This record may retain generic host role labels, command names, branch names,
commit IDs, tag names, PR/check references, verification states, local
operation names, and known limitations. It does not retain private hostnames,
usernames, secrets, credentials, tokens, private keys, session cookies, raw
target data, raw customer data, packet captures, full request or response
bodies, raw runtime artifacts, unredacted evidence bodies, exploit payloads, or
