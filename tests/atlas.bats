...
@test "ci workflow mirrors local Atlas QA gate" {
  ...
  grep -q "manifest's retained release commit" "$ci_doc"
  grep -q 'without requiring every pull request or' "$ci_doc"
  grep -q 'source commit, retention-note commit, or docs-only' "$ci_doc"
  grep -q 'docs-only commit to be a new production candidate' "$ci_doc"
  grep -q 'does not certify the active pull request or source commit as' "$ci_doc"
  ...
  grep -q 'production explainability runs against retained release commit' "$release_trust_workflow"
