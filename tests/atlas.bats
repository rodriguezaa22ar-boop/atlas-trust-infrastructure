...
setup() {
  ...
  chmod +x \
    ...
    "$TEST_ROOT/toolkit/bin/dev-decisions" \
...
@test "M178 approval plane draft defines metadata-only approval governance contract" {
  ...
  docs_index="$TEST_ROOT/toolkit/docs/INDEX.md"
  milestone="$TEST_ROOT/toolkit/docs/retention/milestones/MILESTONE_178.md"
  milestone_index="$TEST_ROOT/toolkit/docs/retention/MILESTONE_INDEX.md"
  combined_text="$TEST_ROOT/m178-approval-plane-text.txt"
  ...
  jq -e '
    all(.approval_workflows[];
      ...
      ((.triggering_adapters | type) == "array") and
      ...
      ((.break_glass | type) == "object") and
      (.break_glass.execution_enabled == false) and
  ...
  jq -e '
    ...
    any(.approval_workflows[]; .id == "atlas.cloud_change_proposal_review" and .live_execution == false and any(.known_limitations[]; contains("proposal-only")) and any(.known_limitations[]; contains("Approval is required before any future state-changing execution"))) and
    any(.approval_workflows[]; .id == "atlas.ticket_transition_proposal_review" and .live_execution == false and any(.known_limitations[]; contains("proposal-only")) and any(.known_limitations[]; contains("No live ticket mutation"))) and
  ...
  grep -q 'APPROVAL_PLANE_M178.md' "$approval_doc"
  grep -q 'M178 is a governance contract, not workflow execution' "$approval_doc"
  grep -q 'Approval records do not grant authorization by themselves' "$approval_doc"
  grep -q 'governance/APPROVAL_PLANE_M178.md' "$docs_index"
  grep -q 'MILESTONE_178.md' "$milestone_index"
  grep -q 'Approval Plane Draft' "$milestone"

  grep -q 'The approval plane is a governance contract, not workflow execution' "$approval_m178_doc"
  grep -q 'M178 does not add approval engine execution' "$approval_m178_doc"
  grep -q 'M178 does not execute approval workflows' "$approval_m178_doc"
  grep -q 'M178 does not add automatic approval' "$approval_m178_doc"
  grep -q 'M178 does not add break-glass execution' "$approval_m178_doc"
  grep -q 'M178 does not add live integrations' "$approval_m178_doc"
  grep -q 'M178 does not add credentials' "$approval_m178_doc"
  grep -q 'API calls' "$approval_m178_doc"
  grep -q 'webhooks' "$approval_m178_doc"
  grep -q 'network collectors' "$approval_m178_doc"
  grep -q 'mutation' "$approval_m178_doc"
  ...
  grep -q 'Approval records do not prove the action was valid' "$approval_m178_doc"
  grep -q 'Approval records do not prove legal compliance' "$approval_m178_doc"
  grep -q 'Approval records do not prove production deployability' "$approval_m178_doc"
  grep -q 'Approval records do not prove complete event coverage' "$approval_m178_doc"
  ...
  grep -q 'AI agents are requesters, not authorities' "$approval_m178_doc"
  grep -q 'lower cost of trust without lowering standards' "$approval_m178_doc"

  ! grep -Eiq 'guaranteed compliance|certified compliant|legally compliant|legally sufficient|compliance guarantee|certification guarantee|external audit complete|enterprise deployment approved|production approval implemented|approval engine implemented|approval workflow execution implemented|automatic approval execution implemented|break-glass execution implemented|live integration enabled|API calls enabled|webhooks enabled|network collectors enabled|credentials configured|mutation authority added|complete event coverage: (true|ready|implemented)|runtime safety proven|model correctness proven|artifact correctness guaranteed|actions outside Atlas cannot happen|Atlas grants authorization' "$combined_text"
  ! grep -Eiq 'Atlas (guarantees|certifies|proves|implements|delivers) (compliance|certification|legal sufficiency|external audit|enterprise deployment approval|production approval|approval engine|approval workflow execution|automatic approval execution|complete event coverage|runtime safety|model correctness|artifact correctness|authorization)' "$combined_text"
...
@test "M182 governance plane integration map connects drafted planes without runtime orchestration" {
  integration_doc="$TEST_ROOT/toolkit/docs/governance/GOVERNANCE_PLANE_INTEGRATION_MAP_M182.md"
  docs_index="$TEST_ROOT/toolkit/docs/INDEX.md"
  milestone="$TEST_ROOT/toolkit/docs/retention/milestones/MILESTONE_182.md"
  milestone_index="$TEST_ROOT/toolkit/docs/retention/MILESTONE_INDEX.md"

  [ -f "$integration_doc" ]
  [ -f "$milestone" ]

  grep -q 'governance/GOVERNANCE_PLANE_INTEGRATION_MAP_M182.md' "$docs_index"
  grep -q '^# Milestone 182: Governance Plane Integration Map$' "$milestone"
  grep -q 'MILESTONE_182.md' "$milestone_index"
  grep -q 'Governance Plane Integration Map' "$milestone_index"

  grep -q 'capabilities.yaml' "$integration_doc"
  grep -q 'adapters/registry.yaml' "$integration_doc"
  grep -q 'policy/policy-plane.yaml' "$integration_doc"
  grep -q 'approval/approval-plane.yaml' "$integration_doc"
  grep -q 'evidence/schemas/evidence-envelope.v1.schema.json' "$integration_doc"

  grep -q 'Capability Manifest' "$integration_doc"
  grep -q 'Adapter Registry' "$integration_doc"
  grep -q 'Policy Plane' "$integration_doc"
  grep -q 'Approval Plane' "$integration_doc"
  grep -q 'Evidence Envelope' "$integration_doc"
  grep -q 'Capability names the action; adapter describes the source/system boundary; policy models the decision; approval models human review; evidence envelope records the proof metadata' "$integration_doc"
  grep -q 'Action request or imported event' "$integration_doc"
  grep -q 'capability lookup' "$integration_doc"
  grep -q 'adapter/source classification' "$integration_doc"
  grep -q 'policy decision model' "$integration_doc"
  grep -q 'approval requirement/state' "$integration_doc"
  grep -q 'evidence envelope' "$integration_doc"
  grep -q 'Plane Responsibility Table' "$integration_doc"
  grep -q 'Current Modeled State' "$integration_doc"
  grep -q 'Future Runtime State' "$integration_doc"

  grep -q 'M182 adds no runtime orchestration' "$integration_doc"
  grep -q 'M182 adds no action router' "$integration_doc"
  grep -q 'M182 adds no runtime policy enforcement' "$integration_doc"
  grep -q 'M182 adds no approval workflow execution' "$integration_doc"
  grep -q 'M182 adds no runtime evidence collection' "$integration_doc"
  grep -q 'M182 adds no adapter execution' "$integration_doc"
  grep -q 'M182 adds no live integrations' "$integration_doc"
  grep -q 'M182 adds no credentials/API calls/webhooks/network collectors' "$integration_doc"
  grep -q 'M182 adds no database/server/web UI' "$integration_doc"
  grep -q 'M182 changes no receipt semantics/hashing/canonicalization/replay behavior' "$integration_doc"

  grep -q 'Human judgment remains required' "$integration_doc"
  grep -q 'Approval records do not prove action validity' "$integration_doc"
  grep -q 'Policy decisions do not grant authorization by themselves' "$integration_doc"
  grep -q 'Evidence envelopes do not replace reviewer judgment' "$integration_doc"

  grep -q 'metadata-only' "$integration_doc"
  grep -q 'raw logs' "$integration_doc"
  grep -q 'secrets' "$integration_doc"
  grep -q 'private keys' "$integration_doc"
  grep -q 'tokens' "$integration_doc"
  grep -q 'Authorization headers' "$integration_doc"
  grep -q 'request bodies' "$integration_doc"
  grep -q 'response bodies' "$integration_doc"
  grep -q 'packet captures' "$integration_doc"
  grep -q 'raw prompts' "$integration_doc"
  grep -q 'raw model outputs' "$integration_doc"
  grep -q 'tool output bodies' "$integration_doc"
  grep -q 'browser/session/cookie material' "$integration_doc"
  grep -q 'customer data' "$integration_doc"
  grep -q 'payment data' "$integration_doc"
  grep -q 'private business records' "$integration_doc"
  grep -q 'unredacted evidence bodies' "$integration_doc"
  grep -q 'raw artifacts' "$integration_doc"

  grep -q 'Example Scenario: AI-Agent Action' "$integration_doc"
  grep -q 'agent is requester, not authority' "$integration_doc"
  grep -q 'does not store raw prompts, raw model outputs, or tool output bodies' "$integration_doc"
  grep -q 'Example Scenario: Release Verification' "$integration_doc"
  grep -q 'release verify action is read-only' "$integration_doc"
  grep -q 'No production approval or external certification is implied' "$integration_doc"
  grep -q 'Example Scenario: Business-Flow Sensitive Change' "$integration_doc"
  grep -q 'No private business records or payment data are embedded' "$integration_doc"
  grep -q 'not legal/business approval by itself' "$integration_doc"

  grep -q 'unknown capability' "$integration_doc"
  grep -q 'unknown adapter' "$integration_doc"
  grep -q 'boundary violation' "$integration_doc"
  grep -q 'approval expired' "$integration_doc"
  grep -q 'approval rejected' "$integration_doc"
  grep -q 'evidence missing' "$integration_doc"
  grep -q 'evidence stale' "$integration_doc"
  grep -q 'evidence unverifiable' "$integration_doc"
  grep -q 'evidence outside Atlas' "$integration_doc"
  grep -q 'unsupported decision' "$integration_doc"

  grep -q 'clearer review' "$integration_doc"
  grep -q 'lower evidence reconstruction work' "$integration_doc"
  grep -q 'safer future connectors' "$integration_doc"
  grep -q 'privacy-preserving governance' "$integration_doc"
  grep -q 'stronger audit readiness' "$integration_doc"
  grep -q 'lower cost of trust without lowering standards' "$integration_doc"
  grep -q 'proof without exposure' "$integration_doc"

  grep -q 'does not prove legal compliance' "$integration_doc"
  grep -q 'does not prove legal sufficiency' "$integration_doc"
  grep -q 'does not prove production deployability' "$integration_doc"
  grep -q 'does not prove external audit completion' "$integration_doc"
  grep -q 'does not prove complete event coverage' "$integration_doc"
  grep -q 'does not prove actions outside Atlas did not happen' "$integration_doc"
  grep -q 'does not prove runtime safety' "$integration_doc"
  grep -q 'does not prove model correctness' "$integration_doc"
  grep -q 'does not prove artifact correctness' "$integration_doc"
  grep -q 'does not create tamper-proof infrastructure' "$integration_doc"
  grep -q 'does not create immutable storage' "$integration_doc"
  grep -q 'does not replace human judgment' "$integration_doc"

  grep -q 'M183 Governance Plane Integration Safety Regression' "$integration_doc"
  grep -q 'M184 Governance Decision Vocabulary' "$integration_doc"
  grep -q 'M186 Receipt/Open-Core Schema Alignment' "$integration_doc"
  grep -q 'M188 Governance Demo Packet' "$integration_doc"
  grep -q 'Atlas lowers the cost of trust without lowering the standard' "$integration_doc"
	  grep -q 'Atlas proves process metadata without becoming the raw-data warehouse' "$integration_doc"
}

@test "M183 governance plane integration safety regression keeps integration map non-runtime and bounded" {
  integration_doc="$TEST_ROOT/toolkit/docs/governance/GOVERNANCE_PLANE_INTEGRATION_MAP_M182.md"
  docs_index="$TEST_ROOT/toolkit/docs/INDEX.md"
  milestone="$TEST_ROOT/toolkit/docs/retention/milestones/MILESTONE_183.md"
  milestone_index="$TEST_ROOT/toolkit/docs/retention/MILESTONE_INDEX.md"

  [ -f "$integration_doc" ]
  [ -f "$milestone" ]
  grep -q 'MILESTONE_183.md' "$milestone_index"
  grep -q 'Governance Plane Integration Safety Regression' "$milestone_index"
  grep -q 'governance/GOVERNANCE_PLANE_INTEGRATION_MAP_M182.md' "$docs_index"
  ...
  grep -q 'Current Modeled State' "$integration_doc"
  grep -q 'Future Runtime State' "$integration_doc"
  grep -q 'M172-M181 define contracts, schemas, examples, docs, and safety regressions' "$integration_doc"

  grep -q 'M182 adds no runtime orchestration' "$integration_doc"
  grep -q 'M182 adds no action router' "$integration_doc"
  grep -q 'M182 adds no runtime policy enforcement' "$integration_doc"
  grep -q 'M182 adds no approval workflow execution' "$integration_doc"
  grep -q 'M182 adds no automatic approval' "$integration_doc"
  grep -q 'M182 adds no automatic escalation' "$integration_doc"
  grep -q 'M182 adds no break-glass execution' "$integration_doc"
  grep -q 'M182 adds no runtime evidence collection' "$integration_doc"
  grep -q 'M182 adds no automatic evidence capture' "$integration_doc"
  grep -q 'M182 adds no evidence collector' "$integration_doc"
  grep -q 'M182 adds no evidence lake implementation' "$integration_doc"
  grep -q 'M182 adds no adapter execution' "$integration_doc"
  grep -q 'M182 adds no live integrations' "$integration_doc"
  grep -q 'M182 adds no credentials/API calls/webhooks/network collectors' "$integration_doc"
  grep -q 'M182 adds no database/server/web UI' "$integration_doc"
  grep -q 'M182 changes no receipt semantics' "$integration_doc"
  grep -q 'M182 changes no hashing/canonicalization/replay behavior' "$integration_doc"

  grep -q 'Human judgment remains required' "$integration_doc"
  grep -q 'Approval records do not prove action validity' "$integration_doc"
  grep -q 'Policy decisions do not grant authorization by themselves' "$integration_doc"
  grep -q 'Evidence envelopes do not replace reviewer judgment' "$integration_doc"
  grep -q 'Supported decisions and unsupported decisions should both be visible' "$integration_doc"
  ...
  grep -q 'agent is requester, not authority' "$integration_doc"
  grep -q 'does not store raw prompts, raw model outputs, or tool output bodies' "$integration_doc"
  grep -q 'does not imply autonomous execution' "$integration_doc"
  grep -q 'Example Scenario: Release Verification' "$integration_doc"
  grep -q 'release verify action is read-only' "$integration_doc"
  grep -q 'No production approval or external certification is implied' "$integration_doc"
  grep -q 'Example Scenario: Business-Flow Sensitive Change' "$integration_doc"
  ...
  ! grep -Eiq 'guaranteed compliance|certified compliant|legally compliant|legal sufficiency guaranteed|compliance guarantee|certification guarantee|external audit complete|automated audit complete|enterprise deployment approved by Atlas|production approval granted|production deployability proven|production readiness achieved|complete event coverage: (true|ready|implemented)|runtime orchestration implemented|runtime enforcement implemented|live integrations enabled|automatic approval enabled|automatic evidence capture enabled|evidence lake implementation added|database-backed truth|immutable storage implemented|tamper-proof infrastructure implemented|raw artifact preservation implemented|evidence sufficiency is automatic' "$integration_doc"
  ! grep -Eiq 'Atlas (guarantees|certifies|proves|implements|delivers) (compliance|certification|legal sufficiency|external audit|automated audit|enterprise deployment approval|production approval|production deployability|production readiness|complete event coverage|runtime orchestration|runtime enforcement|live integrations|automatic approval|automatic evidence capture|evidence lake|database-backed truth|immutable storage|tamper-proof infrastructure|raw artifact preservation|automatic evidence sufficiency)' "$integration_doc"
...
@test "M184 governance decision vocabulary defines shared bounded decision terms" {
  ...
  integration_doc="$TEST_ROOT/toolkit/docs/governance/GOVERNANCE_PLANE_INTEGRATION_MAP_M182.md"
  docs_index="$TEST_ROOT/toolkit/docs/INDEX.md"
  milestone="$TEST_ROOT/toolkit/docs/retention/milestones/MILESTONE_184.md"
  milestone_index="$TEST_ROOT/toolkit/docs/retention/MILESTONE_INDEX.md"
  ...
  jq -e '
    ...
    (term("approval_approved") | ((.known_limitations + .not_proof_of) | any(test("action was valid"))) and ((.known_limitations + .not_proof_of) | any(test("legal sufficiency")))) and
    ...
    (term("receipt_verified") | ((.known_limitations + .not_proof_of) | any(test("external truth")))) and
    (term("replay_verified") | ((.known_limitations + .not_proof_of) | any(test("no events occurred outside Atlas|complete event coverage")))) and
    (term("ready") | ((.known_limitations + .not_proof_of) | any(test("production certification")))) and
    ...
    (term("human_judgment_required") | ((.known_limitations + [.when_to_use]) | any(test("high-risk|ambiguous|unsupported|stale|externally dependent"))))
  ' "$vocabulary"

  grep -q 'governance/GOVERNANCE_DECISION_VOCABULARY_M184.md' "$docs_index"
  grep -q 'MILESTONE_184.md' "$milestone_index"
  grep -q 'Governance Decision Vocabulary' "$milestone_index"
  grep -q 'governance/decision-vocabulary.yaml' "$decision_doc"
  grep -q 'governance/decision-vocabulary.yaml' "$integration_doc"
  ...
  grep -q 'M184 does not add a runtime decision engine' "$decision_doc"
  grep -q 'M184 does not add runtime orchestration' "$decision_doc"
  grep -q 'M184 does not add action routing' "$decision_doc"
  grep -q 'M184 does not add policy enforcement' "$decision_doc"
  grep -q 'M184 does not add approval workflow execution' "$decision_doc"
  grep -q 'M184 does not add evidence collection' "$decision_doc"
  grep -q 'M184 does not add adapter execution' "$decision_doc"
  grep -q 'M184 does not add live integrations' "$decision_doc"
  grep -q 'M184 does not add credentials, API calls, webhooks, network collectors, database/server/web UI, or receipt semantic changes' "$decision_doc"
  grep -q 'M184 changes no hashing/canonicalization/replay behavior' "$decision_doc"
  ...
  ! grep -Eiq 'guaranteed compliance|certified compliant|legally compliant|external audit complete|enterprise deployment approved|production approval granted|production deployability proven|complete event coverage: (true|ready|implemented)|runtime safety proven|model correctness proven|artifact correctness guaranteed|actions outside Atlas cannot happen|tamper-proof infrastructure implemented|immutable storage implemented|automated audit complete|evidence sufficiency is automatic|decision terms grant authorization by themselves' "$decision_doc" "$vocabulary"
  ...
  grep -q 'M185 Governance Decision Vocabulary Safety Regression' "$decision_doc"
  grep -q 'M186 Receipt/Open-Core Schema Alignment' "$decision_doc"
  grep -q 'M188 Governance Demo Packet' "$decision_doc"
...
@test "M186 public source alignment reflects governance stack without overclaiming" {
  readme="$TEST_ROOT/toolkit/README.md"
  ...
  milestone="$TEST_ROOT/toolkit/docs/retention/milestones/MILESTONE_186.md"
  milestone_index="$TEST_ROOT/toolkit/docs/retention/MILESTONE_INDEX.md"
  ...
  for doc in \
    ...
    "$command_ref"; do
    [ -f "$doc" ]
    grep -q 'Atlas records and verifies metadata-only proof chains' "$doc"
    grep -q 'does not grant permission' "$doc"
    grep -q 'replace human judgment' "$doc"
    grep -q 'raw logs' "$doc"
    grep -q 'secrets' "$doc"
    grep -q 'private keys' "$doc"
    grep -q 'tokens' "$doc"
    grep -q 'Authorization headers' "$doc"
    grep -q 'request bodies' "$doc"
    grep -q 'response bodies' "$doc"
    grep -q 'packet captures' "$doc"
    grep -q 'raw prompts' "$doc"
    grep -q 'raw model outputs' "$doc"
    grep -q 'tool output bodies' "$doc"
    grep -q 'browser/session/cookie material' "$doc"
    grep -q 'customer data' "$doc"
    grep -q 'payment data' "$doc"
    grep -q 'private business records' "$doc"
    grep -q 'unredacted evidence bodies' "$doc"
    grep -q 'raw artifacts' "$doc"
  done
  ...
  grep -q 'Atlas Connectors' "$readme"
  grep -q 'Atlas Review' "$readme"
  grep -q 'Atlas Policy' "$readme"
  grep -q 'Atlas Evidence Lake' "$readme"
  grep -q 'docs/governance/CAPABILITY_MANIFEST_M172.md' "$readme"
  grep -q 'docs/governance/ADAPTER_REGISTRY_M174.md' "$readme"
  grep -q 'docs/governance/POLICY_PLANE_M176.md' "$readme"
  grep -q 'docs/governance/APPROVAL_PLANE_M178.md' "$readme"
  grep -q 'docs/governance/EVIDENCE_ENVELOPE_SCHEMA_M180.md' "$readme"
  grep -q 'docs/governance/GOVERNANCE_PLANE_INTEGRATION_MAP_M182.md' "$readme"
  grep -q 'docs/governance/GOVERNANCE_DECISION_VOCABULARY_M184.md' "$readme"
  ...
  grep -q 'Release trust remains essential, but it is no longer the only current priority' "$agents"
  grep -q 'Capability Manifest' "$agents"
  grep -q 'Adapter Registry' "$agents"
  grep -q 'Policy Plane' "$agents"
  grep -q 'Approval Plane' "$agents"
  grep -q 'Evidence Envelope' "$agents"
  grep -q 'Governance Plane Integration Map' "$agents"
  grep -q 'Governance Decision Vocabulary' "$agents"
  ...
  grep -q 'Evidence envelopes are metadata-only schema contracts' "$agents"
  grep -q 'Adapter registry docs do not create live integrations' "$agents"
  grep -q 'autonomous exploitation' "$agents"
  grep -q 'Metadata-Only Packet Rule' "$agents"

  grep -q './bin/dev-governance' "$contributing"
  grep -q './bin/dev-decisions' "$contributing"
  grep -q './bin/dev-evidence' "$contributing"
  grep -q './bin/dev-approval' "$contributing"
  grep -q './bin/dev-policy' "$contributing"
  grep -q './bin/dev-adapters' "$contributing"
  grep -q './bin/dev-capabilities' "$contributing"
  grep -q "nix-shell --run './bin/dev-qa'" "$contributing"
  grep -q './bin/export-public-trust --check' "$contributing"
  grep -q 'metadata-only checks' "$contributing"
  grep -q 'no-overclaim checks' "$contributing"
  grep -q 'reviewer-readable docs' "$contributing"
  ...
  grep -q 'metadata-first proof infrastructure for critical digital actions' "$one_page"
  grep -q 'sits above existing systems' "$one_page"
  grep -q 'create, verify, and replay metadata-only proof receipts' "$one_page"
  grep -q 'lower evidence reconstruction work without lowering standards' "$one_page"
  ...
  [ -f "$milestone" ]
  grep -q 'MILESTONE_186.md' "$milestone_index"
...
@test "M150 AI-agent event proof package bundles reviewer path" {
  ...
  grep -q 'b6a7a9242bcd83d9735488de813bae3e463c6f08' "$milestone"
  grep -q 'PR #55' "$milestone"
  grep -q 'e0fb049504055a0dd0f6d0d90f42726150bba85d' "$milestone"
  grep -q 'docs/reviews/AI_AGENT_EVENT_PROOF_PACKAGE_M150.md' "$milestone"
  grep -q 'PR CodeQL: passed.' "$milestone"
  grep -q 'PR Nix QA: passed.' "$milestone"
  grep -q 'PR Release Trust: passed.' "$milestone"
  grep -q 'Builder post-merge `nix-shell --run' "$milestone"
...
@test "M154 trust claim ladder maps receipts to positive review objectives" {
  ladder="$TEST_ROOT/toolkit/docs/TRUST_CLAIM_LADDER.md"
  ...
  grep -q '^# Atlas Trust Claim Ladder$' "$ladder"
  grep -q '^# Control Objective Mapping$' "$control_map"
  sed -n '1,12p' "$ladder" | grep -q '^Atlas supports stronger review outcomes'
  sed -n '1,12p' "$control_map" | grep -q '^Atlas supports control-objective review'
  ! grep -q 'Atlas does not' "$ladder"
  ! grep -q 'Atlas does not' "$control_map"

  grep -q 'What Atlas Supports' "$ladder"
  grep -q 'Evidence Required' "$ladder"
  grep -q 'What Atlas Verifies' "$ladder"
  grep -q 'What Remains For Reviewers Auditors Authorities' "$ladder"
  grep -q 'Control Objective Coverage' "$ladder"
  grep -q 'Precision Limits' "$ladder"
  grep -q 'Atlas is not shrinking into disclaimers.' "$ladder"
  grep -q 'Boundaries are criteria for stronger claims' "$ladder"
  grep -q 'Every stronger claim requires evidence' "$ladder"
  grep -q 'Atlas verifies the proof envelope' "$ladder"
  grep -q 'receipts, adapters, proof packages, and reviewer workflows' "$ladder"
  grep -q 'generic.external_event.v1' "$ladder"
  grep -q 'Level 0 | Receipt integrity' "$ladder"
  grep -q 'Level 1 | Replayable action record' "$ladder"
  grep -q 'Level 2 | Review-ready event package' "$ladder"
  grep -q 'Level 3 | Control-objective support' "$ladder"
  grep -q 'Level 4 | Evidence sufficiency support' "$ladder"
  grep -q 'Level 5 | External assurance support' "$ladder"
  grep -q 'AI-agent action governance' "$ladder"
  grep -q 'GitHub Actions / CI integrity' "$ladder"
  grep -q 'release governance' "$ladder"
  grep -q 'approval integrity' "$ladder"
  grep -q 'audit readiness' "$ladder"
  grep -q 'business workflow assurance' "$ladder"
  grep -q 'Atlas supports review of AI-agent proposed actions and reported results as event-source metadata' "$ladder"
  grep -q 'Atlas supports review of CI run/check metadata without calling GitHub or embedding raw logs' "$ladder"
  grep -q 'Evidence sufficiency support' "$ladder"
  grep -q 'source-system truth' "$ladder"
  grep -q 'model correctness' "$ladder"
  grep -q 'external audit completion' "$ladder"
  grep -q 'Atlas support is not certification.' "$ladder"
  grep -q 'Atlas support is not external audit completion.' "$ladder"
  grep -q 'Atlas support is not legal compliance.' "$ladder"
  grep -q 'Atlas retained evidence is not tamper-proof infrastructure.' "$ladder"
  grep -q 'Atlas verification is not guaranteed safety proof.' "$ladder"

  grep -q 'AI-agent action governance' "$control_map"
  grep -q 'GitHub Actions / CI integrity' "$control_map"
  grep -q 'release governance' "$control_map"
  grep -q 'approval integrity' "$control_map"
  grep -q 'audit readiness' "$control_map"
  grep -q 'business workflow assurance' "$control_map"
  grep -q 'Review objective' "$control_map"
  grep -q 'Evidence Atlas can provide' "$control_map"
  grep -q 'Verification commands' "$control_map"
  grep -q 'Positive support claim' "$control_map"
  grep -q 'Remaining outside-Atlas determination' "$control_map"
  grep -q 'Atlas supports review of AI-agent activity as metadata-only event-source evidence' "$control_map"
  grep -q 'Atlas supports review of CI run/check metadata as local-file, import-only receipts' "$control_map"
  grep -q 'Atlas supports review of retained release evidence and replayable release-trust state' "$control_map"
  grep -q 'Atlas supports review of approval evidence linked to governed actions' "$control_map"
  grep -q 'Atlas supports cloneable, bounded review of public trust evidence' "$control_map"
  grep -q 'Atlas supports review of workflow evidence coverage without embedding sensitive business data' "$control_map"
  grep -q 'GitHub token-shaped markers' "$control_map"
  grep -q 'raw logs' "$control_map"
  grep -q 'webhook secret markers' "$control_map"
  grep -q 'atlas flow packet' "$control_map"
  grep -q 'atlas flow verify' "$control_map"
  grep -q 'atlas flow assurance' "$control_map"
  grep -q 'atlas op trust-chain' "$control_map"
  grep -q 'public export manifest' "$control_map"
  grep -q 'Atlas support is not certification.' "$control_map"
  grep -q 'Atlas support is not external audit completion.' "$control_map"
  grep -q 'Atlas support is not legal compliance.' "$control_map"
  grep -q 'Atlas retained evidence is not tamper-proof infrastructure.' "$control_map"
  grep -q 'Atlas verification is not guaranteed safety proof.' "$control_map"

  grep -q 'TRUST_CLAIM_LADDER.md' "$docs_index"
  grep -q 'reviews/CONTROL_OBJECTIVE_MAPPING.md' "$docs_index"
  grep -q 'TRUST_CLAIM_LADDER.md' "$control_map"
  grep -q 'RECEIPTS.md' "$ladder"
  grep -q 'GENERIC_EXTERNAL_EVENT_RECEIPT_ADAPTER.md' "$ladder"
  grep -q 'AI_AGENT_EVENT_RECEIPT_PROFILE.md' "$ladder"
  grep -q 'AI_AGENT_EVENT_PROOF_PACKAGE_M150.md' "$ladder"
  grep -q 'GITHUB_ACTIONS_EVENT_PROOF_PACKAGE_M153.md' "$ladder"
  grep -q 'BUSINESS_FLOW_EVIDENCE.md' "$ladder"
  grep -q 'MILESTONE_148.md' "$control_map"
  grep -q 'MILESTONE_152.md' "$control_map"
  grep -q 'business workflow assurance' "$docs_index"

  ! grep -Eiq 'Atlas (certifies|guarantees|is externally audited|is legal compliance|is tamper-proof)' "$ladder"
...
@test "M155 repo hygiene hardens trust claim safety surface" {
  ...
  grep -q 'cachix/install-nix-action@8aa03977d8d733052d78f4e008a241fd1dbf36b3' "$qa_workflow"
...
@test "M168 reviewer quickstart simplifies first review path" {
  quickstart="$TEST_ROOT/toolkit/docs/REVIEWER_QUICKSTART.md"
  docs_index="$TEST_ROOT/toolkit/docs/INDEX.md"
  milestone="$TEST_ROOT/toolkit/docs/retention/milestones/MILESTONE_168.md"
  milestone_index="$TEST_ROOT/toolkit/docs/retention/MILESTONE_INDEX.md"
  sufficiency_report="$TEST_ROOT/toolkit/docs/reviews/EVIDENCE_SUFFICIENCY_REPORT_M158.md"
  decision_packet="$TEST_ROOT/toolkit/docs/reviews/REVIEWER_DECISION_PACKET_M160.md"
  plain_output="$TEST_ROOT/toolkit/docs/reviews/REVIEWER_PLAIN_ENGLISH_OUTPUT_M166.md"
  ...
  grep -q 'REVIEWER_QUICKSTART.md' "$docs_index"
  grep -q 'Start Here' "$quickstart"
  grep -q 'git clone <repo> atlas-review' "$quickstart"
  grep -q "nix-shell --run './tools/atlas/bin/atlas receipt verify examples/receipt/minimal.json'" "$quickstart"
  grep -q "nix-shell --run './tools/atlas/bin/atlas receipt replay examples/receipt/demo-site/\\*.json'" "$quickstart"
  grep -q 'github-actions-run-event.json' "$quickstart"
  grep -q 'github-actions-check-event.json' "$quickstart"
  grep -q 'receipt import-generic-event examples/adapters/generic-external-event/github-actions-run-event.json' "$quickstart"
  grep -q 'receipt verify /tmp/github-actions-run.atlas.json' "$quickstart"
  grep -q 'receipt replay /tmp/github-actions-run.atlas.json /tmp/github-actions-check.atlas.json' "$quickstart"

  grep -q 'Expected output shape' "$quickstart"
  grep -q 'Expected plain-English result' "$quickstart"
  grep -q 'First verify' "$quickstart"
  grep -q 'First replay' "$quickstart"
  grep -q 'receipt: ok' "$quickstart"
  grep -q 'receipt replay: ok' "$quickstart"

  grep -q 'prev_hash' "$quickstart"
  grep -q 'This receipt says it follows the previous event hash' "$quickstart"
  grep -q 'Atlas checked that the receipt hashes match' "$quickstart"
  grep -q 'caller-provided' "$quickstart"
  grep -q 'source-system' "$quickstart"
  grep -q 'truth' "$quickstart"
  ...
  grep -q 'What To Do When Blocked' "$quickstart"
  grep -q 'Nix Shell Missing' "$quickstart"
  grep -q 'Command Failed' "$quickstart"
  grep -q 'Replay Failed' "$quickstart"
  grep -q 'Evidence Missing, Stale, Or Unverifiable' "$quickstart"

  grep -q 'EVIDENCE_SUFFICIENCY_REPORT_M158.md' "$quickstart"
  grep -q 'REVIEWER_DECISION_PACKET_M160.md' "$quickstart"
  grep -q 'REVIEWER_PLAIN_ENGLISH_OUTPUT_M166.md' "$quickstart"
  grep -q 'ORG_CI_RELEASE_REVIEW_WORKFLOW_M164.md' "$quickstart"
  grep -q 'GITHUB_ACTIONS_EVENT_PROOF_PACKAGE_M153.md' "$quickstart"

  for doc in "$quickstart" "$docs_index"; do
    ! grep -Eiq 'certified compliant|guaranteed safe|tamper-proof|externally audited|external audit complete|external SLSA certified|production deployable outside the local Atlas contract|detects all missing events|proves no action happened outside Atlas|model correctness proven|runtime safety proven|artifact correctness guaranteed' "$doc"
    ! grep -Eiq 'Atlas (guarantees|proves|certifies|approves|grants) (compliance|certification|legal sufficiency|complete event coverage|all actions happened|no action happened outside Atlas|business approval|production deployability|external SLSA certification|runtime safety|model correctness|artifact correctness)' "$doc"
  done

  grep -q '^# Milestone 168: Reviewer Quickstart Simplification$' "$milestone"
  grep -q '2b59caa6e9e782b1dcf6ceb37380a766a8357815' "$milestone"
  grep -q 'simplified reviewer quickstart' "$milestone"
  grep -q 'No Atlas runtime behavior changed.' "$milestone"
  grep -q 'Known limitations preserved.' "$milestone"
  grep -q 'atlas-retention-m168' "$milestone"
  grep -q 'MILESTONE_168.md' "$milestone_index"
  grep -q 'Reviewer Quickstart Simplification' "$milestone_index"
...
@test "root README stays a concise landing page with dedicated docs" {
  ...
  grep -q 'Logs say something happened' "$readme"
  grep -q 'proof metadata is well-formed, linked, and replayable' "$readme"
  grep -q '^## Start Here$' "$readme"
  grep -Fq 'New reader: [docs/ATLAS_ONE_PAGE.md]' "$readme"
  grep -Fq 'Demo path: [docs/demo/DEMO_OPERATION.md]' "$readme"
  grep -q 'synthetic/local-safe data only' "$readme"
  grep -q 'evidence and finding links' "$readme"
  ...
  grep -q 'docs/case-studies/CASE_STUDY_VENDOR_PAYMENT_CHANGE.md' "$readme"
  ...
  grep -q 'Case Studies' "$index_html"
  grep -q 'CASE_STUDY_RELEASE_TRUST.md' "$index_html"
  grep -q 'CASE_STUDY_VENDOR_PAYMENT_CHANGE.md' "$index_html"
  ...
  grep -q 'case-studies/CASE_STUDY_VENDOR_PAYMENT_CHANGE.md' "$docs_index"
  ...
  grep -q '^# Case Study: Release Trust$' "$release_trust_case_study"
  grep -q '^## Problem$' "$release_trust_case_study"
  grep -q '^## Current Fragmented Approach$' "$release_trust_case_study"
  grep -q '^## Atlas Proof-Layer Approach$' "$release_trust_case_study"
  grep -q '^## Proof Chain$' "$release_trust_case_study"
  grep -q '^## What Atlas Stores$' "$release_trust_case_study"
  grep -q '^## What Atlas Does Not Store$' "$release_trust_case_study"
  grep -q '^## Artifacts Involved$' "$release_trust_case_study"
  grep -q '^## Verification Commands$' "$release_trust_case_study"
  grep -q '^## What This Proves$' "$release_trust_case_study"
  grep -q '^## What This Does Not Prove$' "$release_trust_case_study"
  grep -q '^## Why This Matters For SLSA-Verifiable Releases$' "$release_trust_case_study"
  grep -q '^## Known Limitations$' "$release_trust_case_study"
  grep -q '^## Responsible-Use Boundary$' "$release_trust_case_study"
  grep -q 'does not replace CI/CD, SLSA tooling, signing tools, artifact' "$release_trust_case_study"
  grep -q 'scanners, SIEMs, or GRC tools' "$release_trust_case_study"
  grep -q 'scoped, retained, metadata-first proof chain' "$release_trust_case_study"
  grep -q 'production-ready under the local Atlas contract' "$release_trust_case_study"
  grep -q 'must not claim SLSA certification' "$release_trust_case_study"
  grep -q 'secrets' "$release_trust_case_study"
  grep -q 'tokens' "$release_trust_case_study"
  grep -q 'credentials' "$release_trust_case_study"
  grep -q 'private keys' "$release_trust_case_study"
  grep -q 'customer data' "$release_trust_case_study"
  grep -q 'target data' "$release_trust_case_study"
  grep -q 'packet captures' "$release_trust_case_study"
  grep -q 'raw request or response bodies' "$release_trust_case_study"
  grep -q 'exploit payloads' "$release_trust_case_study"
  ! grep -q 'SLSA certified' "$release_trust_case_study"
  ! grep -q 'externally audited' "$release_trust_case_study"
  ! grep -q 'tamper-proof' "$release_trust_case_study"
  ! grep -q 'fraud-proof' "$release_trust_case_study"
  ! grep -q 'compliance certified' "$release_trust_case_study"
  ! grep -q 'scanner replacement' "$release_trust_case_study"
  ! grep -q 'autonomous hacking' "$release_trust_case_study"
  ! grep -q '^production-ready$' "$release_trust_case_study"

  grep -q '^# Case Study: Vendor Payment Change$' "$vendor_payment_case_study"
  grep -q '^## Problem$' "$vendor_payment_case_study"
  grep -q '^## Current Fragmented Approach$' "$vendor_payment_case_study"
  grep -q '^## Atlas Proof-Layer Approach$' "$vendor_payment_case_study"
  grep -q '^## Business-Flow Proof Chain$' "$vendor_payment_case_study"
  grep -q '^## What Atlas Stores$' "$vendor_payment_case_study"
  grep -q '^## What Atlas Does Not Store$' "$vendor_payment_case_study"
  grep -q '^## Systems Involved$' "$vendor_payment_case_study"
  grep -q '^## Controls Represented$' "$vendor_payment_case_study"
  grep -q '^## Verification Model$' "$vendor_payment_case_study"
  grep -q '^## What This Demonstrates$' "$vendor_payment_case_study"
  grep -q '^## What This Does Not Demonstrate$' "$vendor_payment_case_study"
  grep -q '^## Known Limitations$' "$vendor_payment_case_study"
  grep -q '^## Responsible-Use Boundary$' "$vendor_payment_case_study"
  grep -q 'does not replace accounting software, bank portals, approval tools' "$vendor_payment_case_study"
  grep -q 'email, document storage, GRC, fraud detection, legal review, or compliance' "$vendor_payment_case_study"
  grep -q 'connects evidence references across those tools into a scoped' "$vendor_payment_case_study"
  grep -q 'retained, metadata-first proof chain' "$vendor_payment_case_study"
  grep -q 'business-flow proof' "$vendor_payment_case_study"
  grep -q 'workflow review evidence' "$vendor_payment_case_study"
  grep -q 'metadata-only operational proof' "$vendor_payment_case_study"
  grep -q 'This demonstrates that Atlas can retain and verify metadata' "$vendor_payment_case_study"
  grep -q 'bank account numbers' "$vendor_payment_case_study"
  grep -q 'routing numbers' "$vendor_payment_case_study"
  grep -q 'payment card data' "$vendor_payment_case_study"
  grep -q 'credentials' "$vendor_payment_case_study"
  grep -q 'tokens' "$vendor_payment_case_study"
  grep -q 'private keys' "$vendor_payment_case_study"
  grep -q 'raw invoices' "$vendor_payment_case_study"
  grep -q 'raw contracts' "$vendor_payment_case_study"
  grep -q 'raw customer data' "$vendor_payment_case_study"
  grep -q 'private business records' "$vendor_payment_case_study"
  grep -q 'unredacted emails' "$vendor_payment_case_study"
  grep -q 'exploit payloads' "$vendor_payment_case_study"
  grep -q 'unauthorized-access instructions' "$vendor_payment_case_study"
  ! grep -q 'fraud-proof' "$vendor_payment_case_study"
  ! grep -q 'guaranteed fraud prevention' "$vendor_payment_case_study"
  ! grep -q 'compliance certified' "$vendor_payment_case_study"
  ! grep -q 'externally audited' "$vendor_payment_case_study"
  ! grep -q 'tamper-proof' "$vendor_payment_case_study"
  ! grep -q 'replaces accounting software' "$vendor_payment_case_study"
  ! grep -q 'replaces bank controls' "$vendor_payment_case_study"
  ! grep -q 'stores bank details' "$vendor_payment_case_study"
...
@test "ci workflow mirrors local Atlas QA gate" {
  ...
  grep -q 'NIX_BUILD_SHELL: bash' "$workflow"
  grep -q 'NIX_BUILD_SHELL: bash' "$slsa_workflow"
  grep -q 'NIX_BUILD_SHELL: bash' "$official_workflow"
  ! grep -q 'NIX_BUILD_SHELL: bash' "$release_trust_workflow"
  grep -q 'retained_nix_path="nixpkgs=$GITHUB_WORKSPACE/nix/nixpkgs.nix"' "$release_trust_workflow"
  [ "$(grep -R -l 'retained_nix_path="nixpkgs=$GITHUB_WORKSPACE/nix/nixpkgs.nix"' "$TEST_ROOT/toolkit/.github/workflows" | wc -l | tr -d ' ')" = "1" ]
  grep -q 'Current source uses shell.nix -> nix/nixpkgs.nix directly' "$release_trust_workflow"
  grep -q 'historical commits may predate M195' "$release_trust_workflow"
  grep -q 'does not restore a moving' "$release_trust_workflow"
  grep -q 'NIX_PATH="$retained_nix_path" nix-shell --run "./tools/atlas/bin/atlas release verify' "$release_trust_workflow"
  ...
  grep -q '`NIX_BUILD_SHELL=bash`' "$ci_doc"
  grep -q '`nix_path: nixpkgs=channel:nixos-unstable`' "$ci_doc"
  grep -q 'Retained historical worktrees may predate M195' "$ci_doc"
  grep -q 'compatibility `NIX_PATH`' "$ci_doc"
  grep -q 'points to the active' "$ci_doc"
  grep -q "checkout's pinned" "$ci_doc"
  ...
  grep -q 'retained release manifest' "$ci_doc"
  grep -q 'selects the retained-evidence tag whose' "$ci_doc"
  grep -q 'production dry-run' "$ci_doc"
  grep -q 'milestone note' "$ci_doc"
  grep -q "whose parent is the manifest's" "$ci_doc"
  grep -q "recorded release commit" "$ci_doc"
  grep -q 'production explainability is' "$ci_doc"
  grep -q 'evaluated where the retained evidence exists' "$ci_doc"
  grep -q 'requiring every pull request, source commit' "$ci_doc"
  grep -q 'retention-note commit, or docs-only' "$ci_doc"
  grep -q 'commit to be a new production candidate' "$ci_doc"
  grep -q 'does not certify the active pull request or source commit' "$ci_doc"
  grep -q 'as production-ready under the local Atlas contract' "$ci_doc"
  ...
  grep -q 'JavaScript/TypeScript CodeQL analysis should be added only when' "$ci_doc"
  grep -q 'intentionally covers the GitHub Actions workflow surface' "$ci_doc"
  ...
  grep -q 'cachix/install-nix-action@8aa03977d8d733052d78f4e008a241fd1dbf36b3' "$release_trust_workflow"
  ...
  grep -q 'evidence_retention_tag=' "$release_trust_workflow"
  grep -q 'candidate_parent="$(git rev-parse "$candidate_commit^"' "$release_trust_workflow"
  grep -q 'git cat-file -e "$candidate_commit:$required_path"' "$release_trust_workflow"
  ...
  ! grep -q 'git checkout -B release-trust-check "$release_commit"' "$release_trust_workflow"
  grep -q 'git checkout -B release-trust-check "$evidence_retention_commit"' "$release_trust_workflow"
  grep -q 'Release Trust verifies retained release evidence from %s for release commit %s' "$release_trust_workflow"
  grep -q 'production explainability runs where retained evidence exists' "$release_trust_workflow"
  grep -q 'does not certify the active PR/source commit as production-ready' "$release_trust_workflow"
  ...
  grep -q 'cachix/install-nix-action@8aa03977d8d733052d78f4e008a241fd1dbf36b3' "$slsa_workflow"
  ...
  grep -q 'git checkout -B main "$ATLAS_RELEASE_COMMIT"' "$slsa_workflow"
  ...
  grep -q 'actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a' "$slsa_workflow"
...
@test "official SLSA generic workflow and claim docs define external verification path" {
  ...
  grep -q 'cachix/install-nix-action@8aa03977d8d733052d78f4e008a241fd1dbf36b3' "$official_workflow"
  ...
  grep -q 'actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a' "$official_workflow"
  grep -q 'base64-subjects: "${{ needs.build.outputs.hashes }}"' "$official_workflow"
  grep -q 'Official SLSA generator v2.1.0 must use the tag ref' "$official_workflow"
  grep -q 'rejects commit SHA refs' "$official_workflow"
  grep -q 'f7dd8c54c2067bafc12ca7a55595d5ee9b75204a' "$official_workflow"
  grep -q 'slsa-framework/slsa-github-generator/.github/workflows/generator_generic_slsa3.yml@v2.1.0' "$official_workflow"
  grep -q 'official_slsa_generator_resolved_commit=f7dd8c54c2067bafc12ca7a55595d5ee9b75204a' "$official_workflow"
  ...
  grep -q 'actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c' "$official_workflow"
  grep -q 'softprops/action-gh-release v2 pinned to immutable commit' "$official_workflow"
  grep -q 'softprops/action-gh-release@b4309332981a82ec1c5618f44dd2e27cc8bfbfda' "$official_workflow"
  ...
  grep -q 'the `v2.1.0` tag ref' "$slsa_doc"
  grep -q 'resolved tag commit' "$slsa_doc"
...
@test "v1 internal RC external review validation records clean clone workflow without overclaiming" {
  ...
  grep -q 'retained production-candidate evidence remains M121' "$review_doc"
  grep -q 'M122 is not a new retained production-candidate state' "$review_doc"
  grep -q 'active M122' "$review_doc"
  grep -q 'branch for external review validation' "$review_doc"
  grep -q 'metadata-only' "$review_doc"
  grep -q 'production-ready under the local Atlas contract' "$review_doc"
  grep -q 'may correctly report `not-ready`' "$review_doc"
  ...
  grep -q './tools/atlas/bin/atlas op trust-chain atlas-dual-node-cockpit --strict' "$review_doc"
